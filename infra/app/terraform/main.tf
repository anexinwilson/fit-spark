...
# Build SA needs to read the runtime config secret to pass it as --build-arg at docker build time
resource "google_secret_manager_secret_iam_member" "build_reads_runtime_config" {
  project   = var.project_id
  secret_id = google_secret_manager_secret.runtime["FITSPARK_RUNTIME_CONFIG_JSON"].secret_id
  role      = "roles/secretmanager.secretAccessor"
